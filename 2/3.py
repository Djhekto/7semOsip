...
def calculate_symbolic_representation_dynamic_system(xdown, xup, ydown, yup, h, leng, G, s_list, pt, myfx, myfy):
    cou = 1
    xtmp = xdown
    ytmp = yup
    xckl = xdown
    ...
    while yckl > ydown:
        while xckl < xup:
            for i in range(0, pt):
                for j in range(0, pt):
                    f1 = myfx
                    f1 = f1.replace("x", enc(xtmp))
                    f1 = f1.replace("y", enc(ytmp))
                    xrz = eval(f1)

                    f2 = myfy
                    f2 = f2.replace("x", enc(xtmp))
                    f2 = f2.replace("y", enc(ytmp))
                    yrz = eval(f2)
...
for gh in range(1, (iterc+1)):
    ...
    G = calculate_symbolic_representation_dynamic_system(x0, x1, y0, y1, h, lengx, G, list_good_dots, pointcounter, strxfunc, stryfunc)
