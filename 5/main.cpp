...
// WNDCLASSA wnd;  //êëàññ îêíà
// HWND hwnd;      //äåñêðèïòîð îêíà
// HDC dc;         //õýíäë êîíòåêñòà
// MSG msg;        //ñîîáùåíèå
...
int main(int argc, char** argv)
{
    ...
    //ðèñîâàíèå
    // hwnd = CreateWindow("class", "window", WS_OVERLAPPEDWINDOW, 0, 0, 1200, 500, 0, 0, 0, 0);
    // dc = GetDC(hwnd);
    ...
    for (long long i=1; i<=num_cells; i++)
    {
        if (global_colors.count(i)==1)
        {
            if (global_colors[i]==2)
            {
                ...
                // fd << 300+(xc_0*scale) << ' ' << 300-(yc_0*scale) << ' ' << 300+((xc_0+u)*scale) << ' '<< 300-((yc_0-u)*scale) << '\n';
                // fd << 300+(xc_0*scale) << ' ' << 300-(yc_0*scale) << ' ' << 300+((xc_0+u)*scale) << ' '<< 300-((yc_0-u)*scale) << '\n';
                fd << xc_0 << ' ' << yc_0-u<< ' ' << u << '\n';
    ...
    for (long long i=num_cells+1; i<=2*num_cells; i++)
    {
        if (global_colors.count(i)==1)
        {
            if (global_colors[i]==2)
            {
                ...
                // fd << 600+(xc_0*scale) << ' ' << 300-(yc_0*scale) << ' ' << 600+((xc_0+u)*scale) << ' ' << 300-((yc_0-u)*scale) << '\n';
                fd << xc_0 << ' ' << yc_0-u<< ' ' << u << '\n';
    ...
    for (long long i=2*num_cells+1; i<=num_cells*3; i++)
    {
        if (global_colors.count(i)==1)
        {
            if (global_colors[i]==2)
            {
                ...
                fd << xc_0 << ' ' << yc_0-u<< ' ' << u << '\n';
                // fd << 900+(xc_0*scale)<< ' ' << 300-(yc_0*scale) << ' ' << 900+((xc_0+u)*scale) <<' '<< 300-((yc_0-u)*scale) << '\n';
