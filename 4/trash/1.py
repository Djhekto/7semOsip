...
#-------------------------------------------------------------------------
    ...
    def mainiteration(self):
        for gh in range(self.flag_from_iterate+1, (self.flag_to_iterate+1)):
            ...
            self.G1 = self.calculate_symbolic_representation_dynamic_system(self.x0, self.x1, self.y0, self.y1, self.h, self.lengx, self.G, self.list_good_dots, self.pointcounter)
            for c in nx.strongly_connected_components(self.G1):
                if len(c) > 0:            
                    #self.G = nx.strongly_connected_components(self.G1)
                    self.G.add_nodes_from(c)
                    print(c)
