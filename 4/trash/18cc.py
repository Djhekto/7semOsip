    ...
    def mainiteration(self):
        for gh in range(self.flag_from_iterate+1, (self.flag_to_iterate+1)):
            ...
            if gh ==  (self.flag_to_iterate):
                ...
                for i in range(10):
                    largest = max(nx.kosaraju_strongly_connected_components(self.G), key=len)
                    print( largest)
                    if len(largest)<2:
                        break
                    self.savekosarajures.append(largest)
                    self.G.remove_nodes_from(list(largest))
                    #for e in largest:
                    #    self.G.remove_nodes_from(e)

                
                
                
                for ii,c in enumerate(nx.kosaraju_strongly_connected_components(self.G)):
