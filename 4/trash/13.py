...
#-------------------------------------------------------------------------
    ...
    def calculate_symbolic_representation_dynamic_system(self, xdown, xup, ydown, yup, h, leng, G, s_list, pt):
        ...
        while yckl > ydown:
            ...
            xtmp = xckl
            ytmp = yckl
#        disconnected_nodes = [node for node in nx.weakly_connected_components(self.G) if len(node) == 1]
        disconnected_nodes = [node for node in nx.weakly_connected_components(self.G) if len(node) <= 2]
        #disconnected_nodes = [node for node in nx.weakly_connected_components(self.G)]
        for node in disconnected_nodes:
            self.G.remove_nodes_from(node)
        print(self.G)
        return
