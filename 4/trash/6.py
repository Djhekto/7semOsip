...
#-------------------------------------------------------------------------
    ...
    def mainiteration(self):
        for gh in range(self.flag_from_iterate+1, (self.flag_to_iterate+1)):
            ...
            if gh == 1:
                print("first")
                self.G = self.calculate_symbolic_representation_dynamic_system1(self.x0, self.x1, self.y0, self.y1, self.h, self.lengx, self.G, self.list_good_dots, self.pointcounter)
    ...
    def calculate_symbolic_representation_dynamic_system1(self, xdown, xup, ydown, yup, h, leng, G, s_list, pt):
        cou = 1
        xtmp = xdown
        ytmp = yup
        xckl = xdown
