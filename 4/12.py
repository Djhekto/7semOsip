...
#-------------------------------------------------------------------------
    ...
    def mainiteration(self):
        for gh in range(self.flag_from_iterate+1, (self.flag_to_iterate+1)):
            ...
            self.calculate_symbolic_representation_dynamic_system(self.x0, self.x1, self.y0, self.y1, self.h, self.lengx, self.G, self.list_good_dots, self.pointcounter)
            ...
            if gh ==  (self.flag_to_iterate):
                ...
                for c in nx.kosaraju_strongly_connected_components(self.G):                        
                    ...
                    if c_count == 1:
                        ...
                        rlcell_list = []
                        for e in alist:
                            rlx0 = self.xposition(e, self.lengx)
                            rly0 = self.yposition(e, self.lengx)
                            rlxrz = rlx0
                            rlyrz = rly0 
                            for i in range(1000):
                                rlxrz = rlxrz + self.xfunc(rlxrz, rlyrz,i)
                                rlyrz = rlyrz + self.yfunc(rlxrz, rlyrz,i)
                                #appendme = [rlxrz, rlyrz]
                                #dotlistlol.append(appendme)
                                cell = m.floor(((self.y1 - rlyrz) / self.h)) * self.lengx + m.ceil((rlxrz - self.x0) / self.h) // 1 + 1
                                print(cell, end=" ")
                                if not (cell in rlcell_list):   
                                    rlcell_list.append(cell)
                        painter.setPen(QColor(255, 0, 0))#
                        painter.setBrush(QColor(255, 0, 0))                
                        for i,e in enumerate(rlcell_list):
