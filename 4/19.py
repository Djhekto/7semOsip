...
#-------------------------------------------------------------------------
            ...
            if gh ==  (self.flag_to_iterate):
                ...
                #for c in nx.strongly_connected_components(self.G):
                   ...
                   #print(makeanumber0to255)
                    painter.setPen(QColor(200, makeanumber0to255, 200))#
                    painter.setBrush(QColor(200, makeanumber0to255, 200))
    ...
    def calculate_symbolic_representation_dynamic_system(self, xdown, xup, ydown, yup, h, leng, G, s_list, pt,gh):
        ...
        for i in range(10):
            ...
            print(self.G)
            if self.G.number_of_nodes() == self.G1.number_of_nodes():
                break


        #self.G1 = self.G.reverse(copy=True)
        #if gh>4:
        #    disconnected_nodes = disconnected_nodes = [node for node in nx.strongly_connected_components(self.G1) if len(node) == 1]
        #    for node in disconnected_nodes:
        #        self.G.remove_nodes_from(node)
        #    print(self.G)
