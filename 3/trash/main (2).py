import time
import matplotlib.pyplot as plt
import networkx as nx
import matplotlib.patches as patches
import math as m

def duf(t, x, y, alpha, beta, omega, k, b):
    return -alpha*x - beta*x**3 - k*y + b*m.cos(omega*t)

def duff(x, y, alpha, beta, k, b, omega):
    if omega*b:
        period = (2*m.pi)/omega
    else:
        period = 1
    h = 0.1
    t = 0
    while t<=period:
        # k1 = y
        # m1 = duf(t, x, y, omega, delta, g)
        # k2 = y + (h*m1)/2
        # m2 = duf(t + h/2, x + (h*k1)/2, y + (h*m1)/2, omega, delta, g);
        # k3 = y + (h*m2)/2;
        # m3 = duf(t + h/2, x + (h*k2)/2, y + (h*m2)/2, omega, delta, g);
        # k4 = y + h*m3;
        # m4 = duf(t + h, x + h*k3, y + h*m3, omega, delta, g);
        # x += (h/6)*(k1+2*k2+2*k3+k4);
        # y += (h/6)*(m1+2*m2+2*m3+m4);
        # t += h
        y += h*duf(t, x, y, alpha, beta, omega, k, b);
        x += h*y;
        t += h;
    return (x, y)

xposition = lambda cell, leng: x0+h*(cell-(cell-1)//leng*leng-1)
yposition = lambda cell, leng: y1-h*((cell-1)//leng+1)

def my_eval(str1):
    str2 = f"lambda x, y: {str1}"
    return eval(str2)

def enc(str1):
    if type(str1)!=type("a"):        str1 = str(str1)
    return "(" + str1 + ")"

def cell_dribling(item, leng):
    lengnew = leng*2
    new1 = 2*item-1+(item-1)//leng*lengnew
    new2 = new1+1
    new3 = new1+lengnew
    new4 = new2+lengnew
    return [new1, new2, new3, new4]

def calculate_symbolic_representation_dynamic_system(xdown, xup, ydown, yup, h, leng, G, pt):
    cou = 1
    xtmp = xdown
    ytmp = yup
    xckl = xdown
    yckl = yup
    cell_list = []
    while yckl > ydown:
        while xckl < xup:
            for i in range(0, pt):
                ytmp -= 1 / pt*h

                for j in range(0, pt):
                    xtmp += 1 / pt*h
                    
                    # xrz = xfunc(xtmp, ytmp)
                    # yrz = yfunc(xtmp, ytmp)
                    # xrz, yrz = duff(xtmp, ytmp, 0.25, 0.3, 1)
                    # xrz, yrz = duff(xtmp, ytmp, -0.15, 0, 0)
                    xrz, yrz = duff(xtmp, ytmp, -0.8, 1, 0.25, 0.3, 1)
                    if xrz < xdown or xrz > xup or yrz < ydown or yrz > yup:
                        continue
                    cell = m.floor(((yup - yrz) / h)) * leng + m.ceil((xrz - xdown) / h) // 1 + 1
                    if not (cell in cell_list):   
                        cell_list.append(cell)

                xtmp = xckl

            xckl += h
            xtmp = xckl
            ytmp = yckl
            # print(cell_list)
            for i in range(0, len(cell_list)):
                G.add_edge(cou, cell_list[i])
            cou += 1
            cell_list.clear()
        yckl -= h
        xckl = xdown
        xtmp = xckl
        ytmp = yckl
    return G

x0 = -1.5
x1 = 1.5
y0 = -1.5
y1 = 1.5
h = 0.5
a = -0.5
b = -0.5

iterc = 6
pointcounter = 10
strxfunc = "x*x-y*y+a"
stryfunc = "2*x*y+b"
symbols = ["x","y","a","b"]

strxfunc = strxfunc.replace(symbols[2], enc(a))
strxfunc = strxfunc.replace(symbols[3], enc(b))
stryfunc = stryfunc.replace(symbols[2], enc(a))
stryfunc = stryfunc.replace(symbols[3], enc(b))

xfunc = my_eval(strxfunc)
yfunc = my_eval(stryfunc)

start_time = time.time()

lengx = abs(x1 - x0) / h
lengy = abs(y1 - y0) / h
list_good_dots = [q for q in range(1, int(lengx*lengy))]
G = nx.DiGraph()

for gh in range(1, (iterc+1)):
    
    if gh <= 7:
        G = calculate_symbolic_representation_dynamic_system(x0, x1, y0, y1, h, lengx, G, pointcounter)
    if gh > 7:
        G = calculate_symbolic_representation_dynamic_system(x0, x1, y0, y1, h, lengx, G, list_good_dots, pointcounter*4)

    list_good_dots = list(G.nodes())  # номера ячеек, которые попали

    if gh ==  (iterc):
        plt.figure(gh)
        plt.title(str(gh)+" iteration")
        plt.xlim(x0, x1)
        plt.ylim(y0, y1)
        plt.grid()
        ax = plt.gca()    
        for c in nx.strongly_connected_components(G):
            if len(c) > 1:
                alist = list(c)
                # print(alist)
                for k in range(0, len(alist)):
                    ss = patches.Rectangle((xposition(alist[k], lengx), yposition(alist[k], lengx)), h, h, color = 'blue', fill=True)
                    ax.add_patch(ss)
        
    G.clear()
    # newbuf = list_good_dots
    # list_good_dots = []
    # for i in range(0, len(newbuf)):
    #     r1 = cell_dribling(newbuf[i], lengx)
    #     list_good_dots += r1
    
    h *= 0.5
    lengx *= 2
    pointcounter -= 1
    print(gh, " iteration is done! Time elapsed: ", (time.time() - start_time))

plt.show()

# x = -0.63293852101279657
# y = 1.127506108119301

# print(duff(x, y, -0.8, 1, 0.25, 0.3, 1))

# G = nx.DiGraph()

# lengx = abs(x1 - x0) / h
# lengy = abs(y1 - y0) / h
# sq = lengx*lengy
# sugar = [q for q in range(1, int(sq)+1)]

# G = calculate_symbolic_representation_dynamic_system(x0, x1, y0, y1, h, lengx, G, pointcounter)

# for elem in nx.strongly_connected_components(G):
#     # print(elem)
#     if len(elem)>1:
#         print(elem)
# #     #     for i in elem:
# #     #         print(type(i), i)